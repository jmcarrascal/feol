...
import FEV1.dif.afip.gov.ar.FECAEACabRequest;
import FEV1.dif.afip.gov.ar.FECAEADetRequest;
import FEV1.dif.afip.gov.ar.FECAEARequest;
import FEV1.dif.afip.gov.ar.FECAEDetRequest;
import FEV1.dif.afip.gov.ar.Tributo;

public class ParseFacturaCaea {

	/**
	 * @param args
	 */
	public static void main(String[] args) {
		FECAEARequest comprobanteType = ParseFacturaCaea
				.parseFacturaABv1("C:/CoinseFE/Coinse.FEDesa/Definitivo/A01000300000001.txt");
		System.out.println("");
	}

	public static FECAEARequest parseFacturaABv1(String rutaArchivo) {
		FECAEARequest feReq = new FECAEARequest();
		...
		FECAEACabRequest feCabecera = new FECAEACabRequest();
		FECAEADetRequest feDetalle = new FECAEADetRequest();
			...
			try {
				...
				if (devolver.substring(0, 1).equals("1")) {
					...
					while (listaParametros.length > i) {
						...
						case 20:
							try {
								feDetalle.setCAEA(listaParametros[i]);
							} catch (Exception e) {}
							break;
						}
						i++;					
	...
	FECAEADetRequest[] detalles = new FECAEADetRequest[1];
