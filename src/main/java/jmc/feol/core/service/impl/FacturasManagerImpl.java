...
public class FacturasManagerImpl implements FacturasManager{
	...
	public List<Factura> getFacturaGrilla(String propertySort, String orderSort, String[] propertyFilter, String[] valueFilter, Integer min,  Integer max) throws Exception {
		...
		facturas = extendedFacturaDAO.getFacturaGrilla(1l,propertySort, orderSort, propertyFilter, valueFilter, min,  max);
	...
	public void reProcesarFactura(String nombreArchivo)  {
		...
		File fileDefinitivo = new File(parametrizacionManager.getParametrizacionByPrimaryKey(Constants.ID_CARPETA_DEFINITIVO).getValor() + nombreArchivo );
	...
	public List<String> getFacturasOrdenadas() {
		...
		List<Factura> facturas = extendedFacturaDAO.getFacturasPerdidas();
		...
		for(Factura factura : facturas){
			if (primer){
				...
				if(factura.getTipo_cbte().equals(tipoComprobanteFijo) && factura.getPunto_vta().equals(prefijoFijo)){
					if((numeroComprobanteAnterior + 1) == factura.getCbt_desde()){
						...
						while(j < 20 && !termino){
							String valor = extendedFacturaDAO.getByPKAfip(prefijoFijo, tipoComprobanteFijo, numeroComprobanteTemp); 												
							if (valor == null || valor.trim().equals("")){
								
								String response = getTransacFacturaRepetida(prefijoFijo, tipoComprobanteFijo, numeroComprobanteTemp , empresa );
								
								String result = prefijoFijo + ";" + tipoComprobanteFijo + ";" +(numeroComprobanteTemp) + ";" + response;
							...
							if (numeroComprobanteTemp.equals(factura.getCbt_desde())){
								termino = true;								
	...
	private String getTransacFacturaRepetida(Integer prefijo, Integer comprobanteTipo, Long nroComprobante, Empresa empresa){
		String result = "";
		Long nroComprobanteAdd = nroComprobante;
			...
			nroComprobanteAdd ++;
				...
				try {
					FECompConsultaResponse feRes =  facturaManager.getComprobanteByPrimary(nroComprobante, comprobanteTipo, prefijo, empresa);
					result = result +  
					feRes.getResultGet().getCodAutorizacion() + ";" + 					
	...
	public void reProcesarFactura(Factura facturaActual) {
		...
		if (facturaActual.getMetodoAfip().equals("LOCAL")){		
			//Copio el Archivo que se encuentra en la carpeta Definitivo en la carpeta provisorio
			fileProvisorio = new File(parametrizacionManager.getParametrizacionByPrimaryKey(Constants.ID_CARPETA_PROVISORIO).getValor() + facturaActual.getNombreArchivo() );
			fileDefinitivo = new File(parametrizacionManager.getParametrizacionByPrimaryKey(Constants.ID_CARPETA_DEFINITIVO).getValor() + facturaActual.getNombreArchivo());
		}else{
			//Copio el Archivo que se encuentra en la carpeta Definitivo en la carpeta provisorio
			fileProvisorio = new File(parametrizacionManager.getParametrizacionByPrimaryKey(Constants.ID_CARPETA_PROVISORIO_EXPOV1).getValor() + facturaActual.getNombreArchivo() );
			fileDefinitivo = new File(parametrizacionManager.getParametrizacionByPrimaryKey(Constants.ID_CARPETA_DEFINITIVO_EXPOV1).getValor() + facturaActual.getNombreArchivo());			
		}
