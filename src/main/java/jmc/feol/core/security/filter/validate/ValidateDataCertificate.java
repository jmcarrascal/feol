package jmc.feol.core.security.filter.validate;
...
import jmc.feol.util.CertificadoProc;
...
public class ValidateDataCertificate {
	...
	private Usuario castCertificadoProcUsuario(CertificadoProc certProc){
		...
		usu.setCargo(certProc.getCargo());
		usu.setEmail(certProc.getEmail());
		
		usu.setNombre(certProc.getCn());
		usu.setOficina(certProc.getOficina());
		usu.setOrganismo(certProc.getO());
		return usu;
	}
