...
public class ReadFilesServiceExpov1 implements Runnable {
	...
	public void run() {
		...
		try {
			...
			.getByPrimaryKey(Constants.ID_USO_LISTENERREADFILESEXPOV1)
			...
			if (useService) {
				...
				if (useService) {
					...
					.getValor() + "/checkFacturasByFileExpov1" ;
					...
					Thread.sleep(valor);		
					System.out.println("Inicio ListenerReadFilesExpo");
					openURL(url);
				}
			}
		} catch (Exception e) {
